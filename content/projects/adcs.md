...
Designed and built a self-contained 1U CubeSat attitude determination and control system as part of SPACE 584 (graduate-level ADCS course).

> **Mission Objective:** Determine spacecraft attitude about a single axis using optical and inertial sensors, actively detumble and stabilize the spacecraft, and image surface features on a scaled Mars analog target at 5 meters range.

The project encompassed full-cycle development from requirements derivation through hardware integration and testbed validation.
...
- **Form Factor:** 10cm x 10cm x ~14cm (1U CubeSat standard)
...
Implemented an **Extended Kalman Filter** in MATLAB with two state variables (angle, angular velocity) for single-axis estimation.
...
> System propagates at **10 Hz** (0.1s intervals) with measurement updates every 2 seconds. Monte Carlo simulations validated filter convergence within sigma-bounds under Gaussian noise assumptions.

### Control Modes

| Mode | Method | Purpose |
|------|--------|---------|
| **Detumble** | B-dot control law using magnetorquers | Reduce angular rates from initial spin-up (tested from 20 deg/s) |
| **Pointing** | Reaction wheel torque commands | Achieve and maintain target attitude. Achieved 0.1 Hz control bandwidth with stable transients |
| **Desaturation** | Magnetorquer-based momentum dumping | Prevent reaction wheel saturation during extended operations |
...
| Metric | Result |
|--------|--------|
| **Estimation** | EKF achieved bounded state estimation with Monte Carlo validation confirming convergence within sigma-bounds |
| **Pointing Accuracy** | **< 1 deg RMS** in testbed runs with 0.1 Hz control bandwidth |
...
| **Attitude Agreement** | Star tracker and sun sensor showed satisfactory agreement (within ~5 deg) during slew tests from -35 deg to +5 deg |
